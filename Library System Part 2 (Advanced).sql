...
-- JOIN issued_status & member tables, after that join the result with books table then join with return status
-- after joining all those tables, we filter books which are returned
...
JOIN
members as m
ON member_id = ist.issued_member_id
JOIN 
books as bk
...
-- to solve this question we can update the books manually or use stored procedures
...
SELECT 
	e.emp_name,
	b.*,
	COUNT(ist.issued_id) as no_book_isssued	
FROM issued_status as ist
JOIN 
employees as e 
ON e.emp_id = ist.issued_emp_id
